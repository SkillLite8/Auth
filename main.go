...
func main() {
	...
	fmt.Println("║     🛸  NeverTime RESOURCE PACK Dumper v5.0 (Bypass) ║")
	...
	fmt.Printf("🛸 Имитируем оригинальный клиент и подключаемся к %s:%d...\n", host, port)

	// Генерация уникальных и валидных данных устройства (Фейк-оригинал)
	deviceId := uuid.New().String()

	clientData := protocol.ClientData{
		DeviceOS:                                7, // Эмуляция Windows 10/11
		...
		DeviceSessionID:                         uuid.New().String(),
	...
	// Делаем паузу перед чтением пакетов, имитируя задержку сетевого стека реального игрока
	time.Sleep(500 * time.Millisecond)

	packs := conn.ResourcePacks()
	if len(packs) == 0 {
		fmt.Println("ℹ️ Сервер не отправил ресурс-паки (возможно, включена жесткая защита Xbox Live Auth).")
		return
	}
	...
	for i, pack := range packs {
		fmt.Printf("   [%d/%d] -> ID: %s... | Размер: %.2f МБ\n", i+1, len(packs), pack.UUID()[:8], float64(pack.Len())/1024/1024)
		
		fileName := filepath.Join(outputDir, fmt.Sprintf("%s.zip", pack.UUID()))
		...
		// Обход Rate Limit: искусственный микро-фриз перед чтением стрима данных
		time.Sleep(300 * time.Millisecond)

		_, err = file.ReadFrom(pack)
		file.Close()

		if err != nil {
			fmt.Printf("❌ Защита сервера оборвала соединение на паке %s: %v\n", pack.UUID()[:8], err)
		} else {
