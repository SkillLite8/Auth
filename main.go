...
func main() {
	...
	fmt.Println("║     🛸  NeverTime RESOURCE PACK Dumper v5.3 (Bypass) ║")
	...
	clientData := login.ClientData{
		...
		DeviceID:         login.DeviceID(deviceId), // Явное приведение к типу login.DeviceID
	...
	for i, pack := range packs {
		...
		// Читаем поток через внутренний метод pack.Reader(), реализующий io.Reader
		packData, err := io.ReadAll(pack.Reader())
		if err != nil {
