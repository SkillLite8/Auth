...
func main() {
	...
	fmt.Println("║     🛸  NeverTime RESOURCE PACK Dumper v5.1 (Bypass) ║")
	...
	// Используем protocol.ClientData напрямую
	...
	for i, pack := range packs {
		// Переводим UUID в строку через .String(), чтобы безопасно её обрезать
		packIDStr := pack.UUID().String()
		shortID := packIDStr[:8]

		fmt.Printf("   [%d/%d] -> ID: %s... | Размер: %.2f МБ\n", i+1, len(packs), shortID, float64(pack.Len())/1024/1024)
		...
		// Получаем байты напрямую из метода .Bytes() плагина gophertunnel
		packData := pack.Bytes()
