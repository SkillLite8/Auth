...
try:
    ...
    print("Установите библиотеки: pip install customtkinter keyboard")
    sys.exit(1)
...
class ClickerCore:
    ...
    def _run_clicker(self):
        ...
        while not self.stop_event.is_set():
            ...
            # Отправка клика (идеально для Minecraft)
            ...
            # Обновляем UI без лагов (не чаще 10 раз в секунду)
            now = time.perf_counter()
            if now - last_ui_update > 0.1 or not self.clicking:
                self.update_ui_callback(current_count)
                last_ui_update = now

            # Идеально точная задержка для ровного CPS
...
class AutoClickerApp(ctk.CTk):
    def __init__(self):
        ...
        self.configure(fg_color="#121317") # Основной темный фон
