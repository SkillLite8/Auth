// Используем динамический eval, чтобы спрятать библиотеку от статического анализатора pkg
const bedrock = eval("require('bedrock-protocol')");
const createClient = bedrock.createClient;
...
class ResourcePackDumper {
  ...
  async init() {
    ...
    this.log('🛸 Запуск Resource Pack Dumper v4.3 (Bypass)', c.bright + c.magenta);
  ...
  createClient() {
    return createClient({
      ...
      encryption: true,
      compression: true,
      raknetBackend: 'js', // Строго используем чистый JS
      clientData: {
        DeviceOS: 7, 
...
console.log(`\n${c.bright}${c.magenta}╔══════════════════════════════════════════════════════╗
║     🛸  NeverTime RESOURCE PACK Dumper v4.3          ║
