...
class ResourcePackDumper {
  ...
  async init() {
    ...
    this.log('🛸 Запуск Resource Pack Dumper v4.0 (Финал)', c.bright + c.magenta);
  ...
  async start() {
    ...
    this.client.on('resource_packs_info', (packet) => {
      ...
      this.client.send('resource_pack_client_response', {
        response_status: 3, // WANT_TO_DOWNLOAD
    ...
    // ЭТАП 3: Имитируем, что всё установили, если сервер требует закрыть сессию красиво
    this.client.on('resource_pack_stack', () => {
      this.client.send('resource_pack_client_response', {
        response_status: 4, // HAVE_ALL_PACKS / COMPLETED
        resource_pack_ids: []
      });
...
console.log(`\n${c.bright}${c.magenta}╔══════════════════════════════════════════════════════╗
║     🛸  NeverTime RESOURCE PACK Dumper v4.0          ║
